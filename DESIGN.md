...
**`enqueue`** inserts into the home CCD's DSQ with vtime, charged from the task's accumulated runtime scaled by weight. Tasks with the interactive signature (high wake frequency, short average runtime, the profile of render threads, compositors, and audio threads) get a bounded vtime credit so they jump the queue within their CCD. This borrows scx_bpfland/scx_lavd's insight without importing their machinery; the credit is clamped so batch tasks can't be starved past the watchdog's patience. The credit alone is not enough when every home CPU is held by long-slice batch work — the wakee is at the head of a queue nobody is dispatching from until a slice expires (measured as pingpong p50 doubling under per-CPU spinner load). So an interactive wakeup also *preempts*: the wakee is inserted directly onto its cache-warm previous CPU with `SCX_ENQ_PREEMPT`, provided that CPU currently runs a non-interactive task (each CPU publishes its current task's class from `running`; without this guard, all-interactive workloads like hackbench would preempt each other on every message) and no more often than `preempt_min_us` per victim CPU (bounding the IPI rate). The displaced batch task re-enqueues with its vtime intact and loses only the remainder of one slice.

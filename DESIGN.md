...
**Fork lineage (BPF, free), severed at exec.** `init_task` fires with `args->fork` set; the child inherits the parent's cohort. Chrome's zygote forks renderers without exec'ing, so that tree lands in one cohort with zero configuration. `exec`, though, is a program-identity boundary: a `sched_process_exec` hook moves the task out of the lineage cohort into a fresh one. An earlier revision let membership survive exec on the theory that the interesting relationships flow through the fork tree — but every process on a desktop descends from one launcher through fork+exec chains, so surviving exec collapses whole sessions (or a whole benchmark harness, where this was first measured as ~38% affinity) into a single machine-wide cohort homed on one CCD, and splits can only repair it after sustained observation. Severing inverts the asymmetry: related processes launched via fork+exec (game ↔ `wineserver`, browser ↔ GPU process) re-merge from their wake edges within about a second of actually talking, while unrelated ones never get grouped in the first place.
...
The ladder is gated on the home CCD having any idle CPU at all, tested with one `bpf_cpumask_intersects` against the idle mask before any of it runs. Under saturation, which is exactly when scheduling cost matters, none of the steps can succeed, and running them anyway costs an SMT-mask fetch, a scratch-mask build and two `scx_bpf_pick_idle_cpu` scans per wakeup purely to fail. Measured under hackbench as roughly a quarter-million wasted double-scans per second. EEVDF spends nothing on the equivalent, via `SIS_UTIL` and the `has_idle_cores` hint, and the absence of that short-circuit was overhead with no policy content. Measured worth: hackbench 15 s to 14 s, closing the gap to EEVDF from ~36% to ~26%. Real but not the dominant cost, which is worth stating because the estimate before measuring was considerably higher. Under load the gate now fires on essentially every miss (`skip` tracks `clamp` within a rounding error), so the in-CCD ladder is close to free during saturation and further batch-throughput gains have to come from somewhere else. The test is deliberately racy, since a CPU freeing up immediately afterwards costs a single placement and not correctness; the task lands in the home DSQ and whichever CPU went idle collects it on its next dispatch. `scx_cohort --monitor` reports the skip count as `skip=`, next to the `clamp=` it feeds.

**`enqueue`** inserts into the home CCD's DSQ with vtime, charged from the task's accumulated runtime scaled by weight. Tasks with the interactive signature (high wake frequency, short average runtime, the profile of render threads, compositors, and audio threads) get a bounded vtime credit so they jump the queue within their CCD. This borrows scx_bpfland/scx_lavd's insight without importing their machinery; the credit is clamped so batch tasks can't be starved past the watchdog's patience. The credit alone is not enough when every home CPU is held by long-slice batch work — the wakee is at the head of a queue nobody is dispatching from until a slice expires (measured as pingpong p50 doubling under per-CPU spinner load). So an interactive wakeup also *preempts*: the wakee is inserted directly onto its cache-warm previous CPU with `SCX_ENQ_PREEMPT`, provided that CPU currently runs a *firmly batch* task — one that rarely wakes (each CPU publishes its current task's wake class from `running`) — and no more often than `preempt_min_us` per victim CPU (bounding the IPI rate). "Firmly batch" is a deliberately stricter test than "not credit-qualified": between the two classes sits frequently-waking long-running work (a hackbench receiver draining a message burst) that fails the interactive test yet carries throughput; an earlier revision victimized that middle class and hackbench ran preemption at the rate-limiter ceiling, +52% elapsed. Spinners and compilers wake rarely and remain valid victims. The displaced batch task re-enqueues with its vtime intact and loses only the remainder of one slice.

Both credit knobs describe behaviour relative to a slice but are stored as absolutes, calibrated against the 5 ms default, so the daemon scales them down to fit a shorter one. Left absolute they invert: at a 750 us slice no task can run for 750 us, let alone clear the 2 ms "short average runtime" ceiling, so every task on the machine reads as interactive, a credit granted universally orders nothing, and a 4 ms credit against a 750 us clamp window flattens what ordering survives. Measured as hackbench taking three times as long as EEVDF while the desktop stayed perfectly responsive — the slice fixed the latency and the miscalibrated credit paid for it. Scaling is one-directional, so an explicitly configured ceiling is never raised and the defaults are untouched at the default slice.

**`dispatch`** on an idle CPU consumes from its own CCD's DSQ first. If empty, it may steal from the other CCD's DSQ, gated by a threshold: steal only when the foreign DSQ has more than `steal_min` waiting tasks *and* its oldest task has waited longer than `steal_delay_us` (a few hundred µs). Both conditions are required — an earlier revision took either alone, and because a queue a few tasks deep is normal operation on a busy CCD (not starvation), two near-full CCDs traded queue heads on nearly every dispatch, re-creating exactly the spreading behavior the gate exists to prevent. Wait time is the real signal that a CCD cannot drain its own queue; a *standing* overload is spill's job to shed, and the gate only smooths transients. No stealing at all would idle half the machine under single-cohort overload before spill reacts and eventually trip the watchdog. A stolen task's cohort assignment does not change; it drifts home on its next wakeup unless the daemon reassigns the cohort.

### 4.3 Load balancing (userspace, ~200 ms tick)

The daemon computes per-CCD load as the sum of member tasks' duty cycles (tracked in BPF via `running`/`stopping` timestamps, read per tick). If the imbalance between CCDs exceeds a threshold (default 20% of one CCD's capacity) for two consecutive ticks, it moves cohorts (smallest-best-repairing first) until the projected gap falls back under the threshold or a per-tick cap is hit (default 8), and writes the new `home_ccd` into the cohort map. Each pick is planned against the projected result of the ones before it, so a batch converges on the balance point rather than every member of it chasing the same stale gap and overshooting into a mirrored imbalance. The cap matters because splits hand every new cohort the parent's home: a seven-way split under hackbench put seven cohorts on one CCD at once, and at the old ceiling of one move per two ticks that took nearly three seconds to unwind, with one CCD above 95% and the other under 20% throughout. Tasks drift to the new home lazily via their next `select_cpu`; nothing is force-migrated, so the transition costs no IPIs.
...
**Chrome:** the zygote forks renderers without exec, so lineage keeps that tree together; the browser, GPU process, and zygote launch through fork+exec and so start as separate cohorts, but their constant Mojo chatter merges them within the first second of use — everything ends up in one cohort with home on, say, CCD0, and Mojo wakeups stay intra-L3. When tab count pushes demand past 8 cores, background renderers (near-zero duty cycle, no recent wake edges to the GPU process) spill to CCD1, while the focused renderer, GPU, and browser processes hold CCD0. The compositor chain never crosses the fabric.

**Wine game:** `steam` → `wine` → game and `wineserver` cross exec boundaries and so start as separate cohorts, but the game ↔ `wineserver` round-trip traffic merges them within a few daemon ticks — one cohort, without dragging in the rest of what Steam has ever launched. Render thread wakes RHI thread via fsync futex with `WAKE_SYNC`; select_cpu places wakee near waker inside the CCD. `wineserver` round-trips stay intra-L3. PipeWire runs outside the lineage tree, but its dense wake edges to the game's audio thread merge it into the cohort within a few daemon ticks. The balancer's residency rule means a momentary background load spike (shader compilation in another Steam process) doesn't evict the game from its CCD.
...
Oversized-cohort spill with sticky selection, interactive vtime credit, SMT preference refinements, TOML rules. Exit criteria: Chrome with 20+ tabs keeps compositor chain on one CCD (verify via per-process CPU masks over time); game frame-time 1% lows ≥ EEVDF and ideally ≥ `scx_lavd`; kernel-compile throughput within 3% of EEVDF; and under a saturating fork-heavy load (`hackbench -g 8 -l 20000`) affinity holds above 80% while both LLCs stay within a factor of two of each other's utilization, with the desktop still usable throughout.

That last one is the spill-side equivalent of the migrations/min flapping check, and it is not hypothetical: spill originally credited every fully idle member a nominal 1 ns toward the excess it needed to shed, so a cohort of mostly-idle members exiled all of them before moving a single hot thread, and an unchecked destination let it push into an LLC already past its own high mark. Measured on a 5950X that drove affinity to 7.5% with one LLC at 9% and the other above 100%, and froze the mouse cursor for seconds at a time. Watch it with `scx_cohort --monitor 1`: the utilizations must converge under load, and `spilled` must track shed load rather than climbing toward the cohort's member count.

### Phase 6: Bench, tune, package (1 week)

Full benchmark sweep against EEVDF, `scx_rusty`, `scx_lavd`, `scx_bpfland`: games (Proton, MangoHud percentiles), Chrome (Speedometer, dropped-frame counts under load), mixed load (game + compile), microbenches (schbench wakeup latencies, hackbench). Tune the numbers that matter: slice length and preemption rate limit first (measured below as dominating interactive latency), then the steal gate, imbalance threshold, residency time, and merge threshold. Package: systemd unit, `scx_loader` integration so it's switchable like the stock schedulers, README with tuning guide. Consider upstreaming to the scx repo; out-of-tree schedulers rot as the kfunc surface evolves, and the compat headers only help if you track the repo.
...
### Measured: slice length dominates interactive latency

The tuning list above was wrong about which numbers matter. A sweep on a 5950X under `hackbench -g 8 -l 20000`, watching both the elapsed time and whether the mouse cursor stalled:

| `--slice-us` | `--preempt-min-us` | hackbench | cursor |
|---|---|---|---|
| 5000 | 5 | 15.0–25.6 s (median 21.1) | freezes for seconds |
| 750 | 5 | 24 s | smooth |
| 750 | 200 | 27 s | smooth |
| 750 | 500 | 19 s | occasional sub-second stalls |
| **2500** | **200** | **15 s** | **smooth** |
| 3500 | 200 | 14 s | occasional sub-second stalls |
| EEVDF | | 11.0–11.3 s | smooth |

Three things came out of this. The slice is the dominant knob for responsiveness, because a wakee waits out whatever slices are queued ahead of it and nothing else in the design shortens that wait; the freeze threshold sits between 2500 and 3500 µs. Preemption is not the lever it was built to be: at 2500/200 it fires around a hundred times per second and the desktop is smooth, while at 5000/5 it fired around 200k times per second and the desktop froze anyway. It earns its keep on spinner-style load, where victims genuinely are firmly batch, and is close to pure overhead under fork-heavy load where almost nothing qualifies. And the old defaults were unstable as well as slow, spanning 15–26 s across five identical rounds against EEVDF's 3% spread.

Those figures predate the `select_cpu` idle-ladder short-circuit (§4.2), which subsequently took 2500/200 from 15 s to 14 s without moving the freeze threshold.

2500/200 is therefore the shipped default: strictly better than the old 5000/5 on both axes, with no tradeoff to weigh. The honest remainder is that 15 s against EEVDF's 11.1 s is roughly 36% off, which misses the parity bar §2 sets and Phase 5 restates, and the sweep shows tuning these two knobs cannot close it, since every faster setting brings the stalls back. Closing it needs a mechanism that separates latency-critical from throughput-critical work, which timing alone cannot do: a compositor and a hackbench sender both wake often and run briefly, so no threshold on either signal tells them apart. The TOML rules are the obvious hook, marking a task interactive explicitly rather than inferring it.
